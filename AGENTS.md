...
- **깊이 조절:** 메인 경로(01–18)는 “개념 + 쓰임 + 최소 코드”.
  기계어·ABI·패딩·재할당 비용 등 **메모리/컴파일 심화**는 본문에 우겨 넣지 말고,
  나중에 `심화` 번호·접힌 카드·별도 섹션으로 추가. (읽기 흐름 보호)
- **약자:** 첫 등장에만 풀네임 한 번.
  예: `STL (Standard Template Library, 표준 템플릿 라이브러리)` → 이후 `STL`.
  매번 풀거나, 아예 안 쓰는 극단은 피함. 사전(19)에도 풀네임 카드 OK.
- 한/영 토글은 나중. 지금은 한국어만.
