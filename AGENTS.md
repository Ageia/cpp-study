...
- **시각화 우선:** 한눈에 이해되는 개념은 가능하면 **SVG · CSS 애니메이션 · 간단한 다이어그램**으로 표현.
  ASCII 도식·긴 문단 설명보다 “보면 이해”를 우선. (예: 스택/힙 쌓임, 포인터 화살표, 호출 스택)
  외부 이미지 파일보다 **인라인 SVG** 선호(단일 HTML 유지). `prefers-reduced-motion` 존중.
  꾸미기만 하는 과한 이펙트는 지양 — 학습 보조가 목적.
- 한/영 토글은 나중. 지금은 한국어만.
